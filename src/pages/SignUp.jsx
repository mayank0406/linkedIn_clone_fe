...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    e.preventDefault();
    try {
      const response = await axios.post(serverUrl + "/api/v1/users/register", userData, { withCredentials: true })
      setErr("");
      setLoading(false);
      setUserData(initial);
  ...
  return (
    <div className='w-full h-screen bg-[white] flex flex-col items-center justify-start gap-[10px]'>
      ...
      <form onSubmit={handleSignUp} className='w-[90%] max-w-[400px] h-[600px] md:shadow-xl flex flex-col justify-center p-[15px] gap-[10px]'>
        <h1 className='text-gray-800 text-[30px] font-semibold m-[30px]'>Sign Up</h1>
        <input type="text" placeholder='firstname' required name="firstName"
          value={userData.firstName}
          onChange={handleChange} className='w-[100%] h-[50px] border-2 border-gray-400 p-[5px] text-gray-800 text-[18px] px-[18px] py-[18px] rounded-md' />
        ...
        <button className='w-[100%] h-[50px] rounded-full bg-[#1C78AA] mt-[30px] text-white mt-[40px]'>{loading ? "loading..." : "Sign Up"}</button>
        <p className='text-center'>Already have an account ? <span className='text-[#1C78AA] cursor-pointer' onClick={() => navigate("/login")}>Sign In</span></p>
      </form>
    </div>
  )
}
